...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, gt_segments, gt_labels, indices, num_boxes):
        ...
        if self.loss_class_type == "ce_loss":
            ...
            if self.use_binary:
                summed_tensor = target_classes_onehot.sum(dim=-1, keepdim=True)
                binary_tensor = (summed_tensor >= 1).to(torch.int64)
                target_classes_onehot = binary_tensor

            # uncertainly mask
            uncertain_mask = torch.ones_like(src_logits, dtype=torch.int64)
            condition = (target_classes_onehot == 0) & (src_logits.sigmoid() > 0.5)
            uncertain_mask[condition] = 0
            ...
            loss_class *= uncertain_mask
            loss_class = loss_class.sum() / (uncertain_mask.sum() + 1e-6) / num_boxes
...
@LOSSES.register_module()
class ZSDefDETRSetCriterion(SetCriterion):
    def loss_boxes(self, outputs, gt_segments, gt_labels, indices, num_boxes):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the IoU loss
        targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
        The target boxes are expected in format (center_x, center_y, w, h), normalized by the image size.
    ...
    def forward(self, outputs, gt_segments, gt_labels, return_indices=False):
        ...
        # Retrieve the matching between the outputs of the last layer and the targets
        outputs_without_aux = {k: v for k, v in outputs.items() if k != "aux_outputs" and k != "enc_outputs"}
        indices = self.matcher(outputs_without_aux, gt_segments, gt_labels, self.use_binary)
        if return_indices:
            indices0_copy = indices
            indices_list = []
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                indices = self.matcher(aux_outputs, gt_segments, gt_labels, self.use_binary)
                if return_indices:
                    indices_list.append(indices)
                for loss in self.losses:
                    l_dict = self.get_loss(loss, aux_outputs, gt_segments, gt_labels, indices, num_boxes)
