...
class SingleStageDetector(BaseDetector):
    ...
    def post_processing(self, predictions, metas, post_cfg, ext_cls, **kwargs):
        rpn_proposals, rpn_scores, segment_labels = predictions
        # rpn_proposals,  # [B,K,2]
        ...
        for i in range(len(metas)):  # processing each video
            ...
            seg_labels = segment_labels[i]

            if num_classes == 1:
                scores = scores.squeeze(-1)
                # labels = torch.zeros(scores.shape[0]).contiguous()
                labels = torch.arange(scores.shape[0]).contiguous()
            else:
            ...
            indices = labels.cpu().detach().numpy().tolist()
            selected_seg_labels = [seg_labels[idx] for idx in indices]

            try:
                thumos_class = list(ext_cls.thumos_class.values())
            except:
                thumos_class = list(ext_cls.cuhk_data_action)
            new_segments = []
            new_labels = []
            new_scores = []
            for item_seg, item_score, item_seg_label in zip(segments, scores, selected_seg_labels):
                if len(item_seg_label) == 4:
                    isl_0, isl_1, isc_0, isc_1 = item_seg_label
                    new_segments.append(item_seg)
                    new_labels.append(thumos_class[int(isl_0)])
                    new_scores.append(item_score * float(isc_0))

                    new_segments.append(item_seg)
                    new_labels.append(thumos_class[int(isl_1)])
                    new_scores.append(item_score * float(isc_1) * 0.5)
                else:
                    isl_0, isc_0 = item_seg_label
                    new_segments.append(item_seg)
                    new_labels.append(thumos_class[int(isl_0)])
                    new_scores.append(item_score * float(isc_0))
            
            new_segments = torch.stack(new_segments)
            new_scores = torch.stack(new_scores)

            sorted_scores, sorted_indices = torch.sort(new_scores, descending=True)
            new_segments = new_segments[sorted_indices]
            sort_list = sorted_indices.cpu().detach().numpy().tolist()
            new_labels = [new_labels[sli] for sli in sort_list]
            new_scores = sorted_scores
