...
class HTadTR(DeformableDETR):
    ...
    def post_processing(self, predictions, metas, post_cfg, ext_cls, **kwargs):
        predict_output, masks = predictions

        all_outputs = []
        o2o_output = {
            "pred_logits": predict_output["pred_logits"],
            "pred_boxes": predict_output["pred_boxes"],
        }
        if self.with_act_reg:
            o2o_output["pred_actionness"] = predict_output["pred_actionness"]
        all_outputs.append(o2o_output)
        o2m_output = {
            "pred_logits": predict_output["pred_logits_one2many"],
            "pred_boxes": predict_output["pred_boxes_one2many"],
        }
        if self.with_act_reg:
            o2m_output["pred_actionness"] = predict_output["pred_actionness_one2many"]
        all_outputs.append(o2m_output)
