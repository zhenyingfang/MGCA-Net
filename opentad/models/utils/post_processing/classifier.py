...
class CUHKANETClassifier:
    def __init__(self, path, topk=1, test_class_map=None):
        super().__init__()
    ...
    def __call__(self, video_id, segments, scores):
        ...
        for k in range(len(cuhk_classes)):
            if cuhk_classes[k] in self.ext:
                ...
                if need_k > 0:
                    new_scores.append(scores * (cuhk_score[k] * 0.5))
                else:
...
class UntrimmedNetTHUMOSClassifier:
    ...
    def __call__(self, video_id, segments, scores):
        ...
        if len(self.ext) > 0:
            unet_classes, unet_scores = [], []
            need_k = 0
            for k in range(20):
                if self.thumos_class[self.thu_label_id[video_cls_rank[-k - 1][1]] + 1] in self.ext:
                    unet_classes.append(self.thu_label_id[video_cls_rank[-k - 1][1]] + 1)
                    if need_k == 0:
                        unet_scores.append(video_cls_rank[-k - 1][0])
                    else:
                        unet_scores.append(video_cls_rank[-k - 1][0] * 0.5)
                    need_k += 1
                if need_k >= self.topk:
                    break
        else:
