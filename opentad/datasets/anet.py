...
class AnetResizeDataset(ResizeDataset):
    def get_gt(self, video_info, thresh=0.01):
        ...
        for anno in video_info["annotations"]:
            ...
            if anno["label"] in self.class_map:
                if (not self.filter_gt) or (gt_scale > thresh):
                    gt_segment.append([gt_start, gt_end])
                    if self.class_agnostic:
                        gt_label.append(0)
                        real_label.append(self.real_class_map.index(anno["label"]))
                    else:
        ...
        if len(gt_segment) == 0:  # have no valid gt
            ...
            annotation = dict(
                ...
                gt_real_labels= np.array(real_label, dtype=np.int32),
            )
    ...
    def __getitem__(self, index):
        ...
        results = self.pipeline(
            dict(
                ...
                class_map_idx=self.class_map_idx,
                **video_anno,
            )
        )
        return results
