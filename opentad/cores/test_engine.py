...
def eval_one_epoch(
    ...
    for data_dict in tqdm.tqdm(test_loader, disable=(rank != 0)):
        ...
        if not isinstance(results, list):
            ...
            for k, v in results[1].items():
                if k in result_dict_o2m.keys():
                    result_dict_o2m[k].extend(v)
                else:
                    result_dict_o2m[k] = v

    result_dict = gather_ddp_results(world_size, result_dict, cfg.post_processing)
    if isinstance(results, list):
        result_dict_o2m = gather_ddp_results(world_size, result_dict_o2m, cfg.post_processing)
    ...
    if rank == 0:
        ...
        if isinstance(results, list):
            result_dict_o2m = dict(results=result_dict_o2m)
            if cfg.post_processing.save_dict:
                result_path_o2m = os.path.join(cfg.work_dir, "result_detection_o2m.json")
                with open(result_path_o2m, "w") as out:
                    json.dump(result_dict_o2m, out, indent=4)
